...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, W):
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            xt, attn_map_t = self.temporal_attn(self.temporal_norm1(xt))
            attn_map_t = rearrange(attn_map_t, '(b p) h t1 t2 -> b p h t1 t2', b=B,p=196,h=12,t1=8,t2=8)
            res_temporal = self.drop_path(xt)
            ...
            xs, attn_map_s = self.attn(self.norm1(xs))
            attn_map_s = rearrange(attn_map_s, '(b t) h d1 d2 -> b t h d1 d2',b=B,t=8,h=12,d1=197,d2=197)
            res_spatial = self.drop_path(xs)
            ...
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x, attn_map_s, attn_map_t
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            x, attn_map_s, attn_map_t = blk(x, B, T, W)
            attn_map_s_list.append(attn_map_s.unsqueeze(0))
        attn_map_s = torch.cat(attn_map_s_list, dim=0)
        attn_map_s = rearrange(attn_map_s, 'l b t h n m -> b t l h n m')
    ...
    def forward(self, x):
        x, attn_map_s, attn_map_t = self.forward_features(x)
        return x, attn_map_s, attn_map_t
...
class TimeSformer(nn.Module):
    ...
    def forward(self, x):
        x, attn_map_s, attn_map_t = self.model(x)
        return x, attn_map_s, attn_map_t
